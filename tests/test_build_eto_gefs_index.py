...
def test_gefs_case_index_rejects_unknown_station_before_writing(tmp_path: Path) -> None:
    """An unknown station selector must fail before creating the index."""
    root = tmp_path / "archive"
    candidates = root / "candidates"
    candidate = candidates / "2019070300"
    candidate.mkdir(parents=True)
...
def test_station_selector_is_repeatable_and_deterministic() -> None:
    """Repeated selectors produce one sorted station sequence."""
