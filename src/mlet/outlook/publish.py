...
<style>
:root{{--paper:#FAFAF8;--surface:#FFFFFF;--surface-2:#F2F2EF;--ink:#191D1B;--ink-2:#505652;--ink-3:#7C827E;--line:#E0E0DB;--line-strong:#BFBFB8;--accent:#17706B;--warn-bg:#FBF3E4;--warn-border:#C77E1B;--warn-ink:#7A4A08;--sans:"Söhne","Sohne","Inter",-apple-system,"Helvetica Neue",sans-serif;--mono:"Söhne Mono","IBM Plex Mono",ui-monospace,SFMono-Regular,monospace}}
@media(prefers-color-scheme:dark){{:root{{--paper:#111413;--surface:#191D1B;--surface-2:#212624;--ink:#E9EAE7;--ink-2:#A8AEA9;--ink-3:#787E7A;--line:#2B302D;--line-strong:#454B47;--accent:#4CB8AE;--warn-bg:#2B2213;--warn-ink:#E5B36A}}}}
*{{box-sizing:border-box}}body{{font-family:var(--sans);margin:0;color:var(--ink);background:var(--paper);font-size:15px;line-height:1.6;-webkit-font-smoothing:antialiased}}
main{{max-width:1100px;margin:auto;padding:0 24px 64px}}
.topbar{{display:flex;justify-content:space-between;align-items:center;padding:16px 0;border-bottom:1px solid var(--line);margin-bottom:32px;font-family:var(--mono);font-size:13px;font-weight:600}}
.topbar .unit{{font-weight:400;color:var(--ink-3);font-size:12px}}
h1{{font-size:clamp(24px,3.4vw,34px);font-weight:600;letter-spacing:-.02em;line-height:1.15;margin:0 0 16px}}
h2{{font-size:15px;font-weight:600;text-transform:uppercase;letter-spacing:.07em;color:var(--ink-2);margin:40px 0 10px}}
.status{{display:block;background:var(--warn-bg);border:1px solid var(--warn-border);border-radius:6px;padding:12px 15px;font-size:13.5px;font-weight:600;color:var(--warn-ink);margin:0 0 16px}}
.chips{{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 16px}}
.chip{{display:inline-flex;gap:7px;align-items:center;font-family:var(--mono);font-size:12px;background:var(--surface);border:1px solid var(--line);border-radius:3px;padding:4px 9px;color:var(--ink-2)}}
.chip .dot-pending{{width:6px;height:6px;border-radius:50%;background:var(--warn-border)}}
.warning{{font-weight:600;font-size:13.5px;margin:0 0 8px}}
.note{{font-size:13.5px;color:var(--ink-2);max-width:72ch;margin:0 0 20px}}
.controls{{display:flex;gap:16px;flex-wrap:wrap;margin:0 0 16px}}
label{{display:inline-block;font-size:12.5px;color:var(--ink-2)}}
select{{display:block;margin-top:4px;font-family:var(--sans);font-size:13.5px;padding:7px 10px;border:1px solid var(--line-strong);border-radius:6px;background:var(--surface);color:var(--ink)}}
svg{{width:100%;height:430px;background:var(--surface-2);border:1px solid var(--line);border-radius:6px}}
.dot{{stroke:var(--paper);stroke-width:1}}
.legend{{margin:10px 0 16px;max-width:420px}}
.legend .ramp{{height:10px;border-radius:3px;border:1px solid var(--line);background:linear-gradient(90deg,#440154,#414487,#2A788E,#22A884,#7AD151,#FDE725)}}
.legend .lbl{{display:flex;justify-content:space-between;font-family:var(--mono);font-size:11px;color:var(--ink-3);margin-top:4px}}
#detail{{font-family:var(--mono);font-size:12.5px;line-height:1.8;background:var(--surface);padding:12px 15px;border:1px solid var(--line);border-radius:6px;color:var(--ink-2)}}
ul{{margin:0;padding-left:20px;font-size:13.5px;color:var(--ink-2)}}li{{margin:4px 0}}
small{{font-family:var(--mono);font-size:11.5px;color:var(--ink-3)}}
</style></head><body><main>
<div class="topbar"><span>MLET·Idaho ET Outlook</span><span class="unit">mm/day · native weather grid</span></div>
<h1>Idaho regional ET outlook</h1><p class="status" role="status">{html.escape(fixture_notice)}</p>
<div class="chips"><span class="chip">run {html.escape(str(candidate['run_id']))}</span><span class="chip">issued {html.escape(str(candidate['issued_at']))}</span><span class="chip">native weather grid</span><span class="chip"><span class="dot-pending"></span>validation_pending</span><span class="chip">promotion: false</span></div>
<p class="warning">{html.escape(_REGIONAL_WARNING)}</p>
<p class="note">Weather-grid reference points are shown only when retained by the source contract; they are not field boundaries or synthetic cell polygons. Uncertainty is shown as p10, p50, and p90 where applicable.</p>
<div class="controls"><label>Layer <select id="layer">{options}</select></label>
<label>Date <select id="date"></select></label><label>Quantile <select id="quantile"><option>p10</option><option selected>p50</option><option>p90</option></select></label></div>
<svg id="map" viewBox="0 0 900 430" role="img" aria-label="Native weather-grid reference-point map"></svg>
<div class="legend"><div class="ramp"></div><div class="lbl"><span>0 mm/day</span><span>viridis</span><span>8+ mm/day</span></div></div>
<div id="detail"></div>
